...
class EquipmentShopScene extends BaseScene {
    ...
    create() {
        ...
        for (let i = 0; i < Item.swords.length; i++) {
            ...
            const costText = this.add.text(button.x, button.y + 100, Item.swords[i].cost.toString());
        }
        for (let i = 0; i < Item.armors.length; i++) {
            const button = this.add
                .sprite(150 + Item.swords.length * 150 + 125 * i, 400, Item.armors[i].name)
                .setOrigin(0, 0)
                .setScale(100 / 512, 100 / 512)
                .setInteractive({ cursor: "pointer" })
                .on(Phaser.Input.Events.POINTER_OUT, () => {
                ...
                .on(Phaser.Input.Events.POINTER_OVER, () => {
                this.invokeTooltip(150 + Item.swords.length * 150 + 125 * i, 375, Item.armors[i].tooltip());
            })
                .on(Phaser.Input.Events.POINTER_DOWN, () => {
            ...
            const costText = this.add.text(button.x, button.y + 100, Item.armors[i].cost.toString());
        }
    }
    updateMoneyText() {
